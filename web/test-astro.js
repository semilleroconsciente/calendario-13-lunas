...
console.log('--- Luna sale/puesta Penco (aprox ±15 min) ---');
for (const d of ['2026-08-28', '2026-09-10', '2026-09-11', '2026-12-21']) {
  const [y, m, dd] = d.split('-').map(Number);
  const mr = cal.moonForDay(Date.UTC(y, m - 1, dd, 12));
  const f = (x) => (x ? fmtT.format(new Date(x)) : '--');
  console.log(`${d}: sale ${f(mr.rise)} – puesta ${f(mr.set)}`);
}
