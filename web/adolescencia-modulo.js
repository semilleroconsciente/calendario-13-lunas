...
function buildDialog() {
  ...
  var body =
    ...
    cuerpoHTML +
    '<div class="menstrual-card"><h4>🔗 Conecta con tu calendario</h4><p class="muted" style="font-size:11px">Lleva tu energía y descanso en 📓 Mi espacio. Si menstruas, usa también 🌸 Ciclo para predecir y entender tu ánimo por fase.</p><div style="display:flex;gap:6px;flex-wrap:wrap"><button type="button" id="adolGoCiclo" class="btn" style="width:auto">🌸 Ir a Ciclo</button><button type="button" id="adolGoRutina" class="btn" style="width:auto">🌞 Mi ritmo</button><button type="button" id="adolGoGym" class="btn" style="width:auto">💪 Entrenamientos</button></div></div>' +
    '</div>' +
...
function setup() {
  /* 1) inyectar botón en grupo Aprender > Infancias */
  try {
    if (!$('btnAdolescencia')) {
      ...
      if (g) {
        ...
        try { btn.setAttribute('data-sub', 'infancias'); } catch (eS) {}
      ...
      try {
        var curA = $('btnAdolescencia');
        var curGA = curA.closest ? curA.closest('.action-group') : null;
        var curNA = curGA && curGA.getAttribute ? curGA.getAttribute('data-group') : null;
        if (curNA && curNA !== 'aprender') {
          var gdA = document.querySelector('.action-group[data-group="aprender"] .group-btns');
          if (gdA) { gdA.appendChild(curA); try { curA.setAttribute('data-sub', 'infancias'); } catch (eS) {} }
        }
      } catch (eM) {}
    }
  } catch (e) {}
  ...
  var bridges = [['adolGoCiclo', 'btnMenstrual'], ['adolGoRutina', 'ritmoPlan'], ['adolGoGym', 'btnGym'],
