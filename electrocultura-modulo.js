...
function setup() {
  /* 1) inyectar botón en grupo Herramientas (migra desde Territorio si existía) */
  try {
    var existing = $('btnElectrocultura');
    if (existing) {
      var oldG = existing.closest ? existing.closest('.action-group') : null;
      var inHerr = oldG && oldG.getAttribute && oldG.getAttribute('data-group') === 'herramientas';
      if (!inHerr) { try { existing.remove(); } catch (e0) { try { existing.parentNode.removeChild(existing); } catch (e1) {} } existing = null; }
    }
    if (!existing) {
      var g = document.querySelector('.action-group[data-group="herramientas"] .group-btns');
      if (g) {
  ...
  /* 3) checkbox en configDialog (grupo Herramientas; limpia resto de Territorio) */
  try {
    try {
    ...
    if (!document.querySelector('#configDialog input[data-btn="btnElectrocultura"]')) {
      ...
      groups.forEach(function (gr) {
        var h = gr.querySelector('h5');
        if (h && h.textContent.indexOf('Herramientas') >= 0) {
